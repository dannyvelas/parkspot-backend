...
func (suite carRouterSuite) TestEdit_CarDNE_Negative() {
	request := models.Car{ID: "9b6d89a6-0b66-4170-be8d-eba43f8bf478", LicensePlate: "NEWLP"} // id that doesn't exist

	endpoint := fmt.Sprintf("%s/api/car", suite.testServer.URL)
...
func (suite carRouterSuite) TestEdit_Car_Positive() {
	...
	tests := map[string]test{
		"licensePlate":              {request: models.Car{ID: testCar.ID, LicensePlate: "NEWLP"}},
		"licensePlate, color":       {request: models.Car{ID: testCar.ID, LicensePlate: "NEWLP", Color: "NEWCOLOR"}},
		"licensePlate, color, make": {request: models.Car{ID: testCar.ID, LicensePlate: "NEWLP", Color: "NEWCOLOR", Make: "NEWMAKE"}},
		"model":                     {request: models.Car{ID: testCar.ID, Model: "NEWMODEL"}},
	}
	...
	executeTest := func(test test) error {
		...
		suite.Equal(test.expected.ID, carResp.ID)
