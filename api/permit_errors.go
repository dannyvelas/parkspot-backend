...
var (
	errActivePermit = fmt.Errorf("Cannot create a permit during these dates" +
		" because this car has at least one active permit during that time.")
	errNoResident = fmt.Errorf("Users must have a registered account to request a" +
		" guest parking permit. Please create their account before requesting their permit.")
	errPermitTooLong = fmt.Errorf("Error: Requests cannot be longer than %d days,"+
		...
		config.MaxPermitLength)
	errPermitDateIntersect = fmt.Errorf("Cannot create a permit during these dates" +
		" because this resident has at least two active permits during that time.")
)

func errEntityDaysTooLong(entity string, amtDaysUsed int) error {
	entityLower := cases.Lower(language.English).String(entity)
	entityTitle := cases.Title(language.English).String(entity)

	return fmt.Errorf("Error: This %s has used parking permits that have lasted"+
		...
		amtDaysUsed,
		entityTitle,
		config.MaxParkingDays,
		entityLower)
}

func errPermitPlusEntityDaysTooLong(entity string, amtDaysUsed int) error {
	entityLower := cases.Lower(language.English).String(entity)

	return fmt.Errorf("Error: This request would exceed the %s's"+
