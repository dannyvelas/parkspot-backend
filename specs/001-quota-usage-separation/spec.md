...
- **FR-001**: System MUST calculate each resident's days-used value solely as the sum of days across that resident's permits that currently count toward their limit (excluding permits explicitly exempted from the limit and excluding residents flagged for unlimited days); this value MUST NOT be directly settable by any user, including administrators.
...
- **FR-004**: System MUST treat allowance adjustments as additive, permanent, individually-retained records — creating a new adjustment MUST NOT overwrite or remove any prior adjustment for that resident.
