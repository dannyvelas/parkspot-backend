...
**Decision**: `storage/psql/database.go`'s `CreateSchemas()` currently hardcodes `migrator.Migrate(1)` — i.e., integration tests (`testcontainers`-backed `NewSandboxDatabase`) only ever apply migration `000001_schemas`, deliberately skipping the seed-data migrations `000002`–`000006`. Because this feature's new table is genuine schema (not seed data), `CreateSchemas()` must be updated to `migrator.Migrate(8)` (the new final schema-only version, after `000007`/`000008` above) so integration tests see `quota_adjustment` and the dropped column, while still skipping the unrelated seed migrations.

**Rationale**: Discovered by reading `storage/psql/database.go` and `storage/psql/test_helpers.go` — every repo-level and app-level integration test in this repo (`app/permit_test.go`, `api/car_handler_test.go`, etc.) depends on `CreateSchemas()` for its Postgres schema, so this is a required, non-optional part of the implementation, not an incidental detail.

**Alternatives considered**: Insert the new table directly into `000001_schemas.up.sql`. Rejected — that migration has already been applied to any real deployed database; editing an already-applied migration file breaks `golang-migrate`'s checksum/ordering guarantees and is not how any other schema change in this repo's history has been done (every later table added its own numbered migration).
