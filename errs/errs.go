...
var (
	EmptyFields          = ApiErr{"One or more missing fields"}
	InvalidFields        = ApiErr{"One or more invalid fields"}
	Unauthorized         = ApiErr{"unauthorized"}
	NotFound             = ApiErr{"not found"}
	AlreadyExists        = ApiErr{"already exists"}
	ResidentForPermitDNE = ApiErr{"Users must have a registered account to request" +
		" a guest parking permit. Please create their account before requesting their permit."}
	CarForPermitDNE = ApiErr{"The car that you chose for this permit does not" +
		" exist. Please create or choose another car."}
	CarActivePermit = ApiErr{"Cannot create a permit during these dates" +
	...
	ResidentTwoActivePermits = ApiErr{"Cannot create a permit during these dates" +
